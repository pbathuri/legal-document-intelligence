...
class InstrumentFact(BaseModel):
    doc_id: str = ""
    doc_label: str = ""
    doc_type: str = Field(default="unknown", description="sale deed, gift deed, partition, mutation extract, EC, RoR, RTC, tax receipt, layout approval, other")
    execution_date: str | None = None
    registration_date: str | None = None
    seller_names: list[str] = Field(default_factory=list)
    buyer_names: list[str] = Field(default_factory=list)
    parcel_ids: list[str] = Field(default_factory=list, description="survey/plot/khata/patta/ULPIN/CTS references")
